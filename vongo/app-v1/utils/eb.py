...
def create_rule(
    name, event_bus, event_pattern=None, schedule_expression=None, enabled=True
) -> dict:
    try:
...
def delete_rule(name, event_bus):
    try:
        events.delete_rule(Name=name, EventBusName=event_bus, Force=True)
    except Exception as e:
...
def create_schedule(name, expression, target, role, enabled=True, start=None, end=None):
    params = {
        "FlexibleTimeWindow": {"MaximumWindowInMinutes": 5, "Mode": "FLEXIBLE"},
        "Name": name,
...
def remove_targets(rule, event_bus):
    try:
        events.remove_targets(Rule=rule, EventBusName=event_bus, Ids=[rule + "-target"])
    except Exception as e:
