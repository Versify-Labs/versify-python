...
def before(func):
    """
    JourneyRun a function before the handler is invoked, is passed the event & context
...
def after(func):
    """
    JourneyRun a function after the handler is invoked, is passed the response
...
def on_exception(func):
    """
    JourneyRun a function when a handler thows an exception. It's return value is
    returned to AWS.
...
def async_handler(handler):
    """
    This decorator allows for use of async handlers by automatically journey_running
    ...
    *NOTE: Python 3 only*
    ...
    def wrapper(event, context):
        ...
        return context.loop.journey_run_until_complete(handler(event, context))
...
def user_required(func):
    ...
    def wrapper(event, context):
        ...
        # If the user is logged in, journey_run the function
        return func(event, context)
...
def account_required(func):
    ...
    def wrapper(event, context):
        ...
        if token_type == "basic":
            ...
            event["requestContext"]["authorizer"] = {"account": account_id}
