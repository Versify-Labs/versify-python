...
class ContactService(ExpandableResource):
    ...
    def create(self, body: dict) -> dict:
        ...
        contacts = self.list({"account": body["account"], "email": body["email"]})
        if len(contacts) > 0:
            contact = contacts[0]
    ...
    def delete(self, contact_id: str) -> bool:
        ...
        deleted = self.collection.find_one_and_delete({"_id": contact_id})
    ...
    def count_segment_contacts(self, account_id, vql):
        stages = [vql_stage(vql=vql, account=account_id), {"$count": "count"}]
        cursor = self.collection.aggregate(stages)
