...
class WebhookEventService(ExpandableResource):
    ...
    def get(self, webhook_event_id: str) -> dict:
        ...
        Returns:
            dict: The webhook_event.
        """
        logging.info("Retrieving webhook_event", extra={"id": webhook_event_id})

        # Find document matching filter
        webhook_event = self.collection.find_one(filter={"_id": webhook_event_id})
    ...
    def delete(self, webhook_event_id: str) -> bool:
        ...
        deleted = self.collection.find_one_and_delete({"_id": webhook_event_id})
        if not deleted:
            raise NotFoundError
