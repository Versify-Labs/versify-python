...
from ..config import AirdropConfig
...
class AirdropService(ExpandableResource):
    def __init__(
        ...
        self.collection = mdb[AirdropConfig.db][AirdropConfig.collection]
        self.expandables = AirdropConfig.expandables
        self.Model = AirdropConfig.model
        self.object = AirdropConfig.object
        self.prefix = AirdropConfig.prefix
        self.search_index = AirdropConfig.search_index
    ...
    def create(self, body: dict) -> dict:
        ...
        Returns:
            dict: The airdrop.
        """
        logging.info("Creating airdrop", extra={"body": body})
        ...
        body["email_settings"]["from_image"] = account["branding"].get("logo")
        body["email_settings"]["from_name"] = account["name"]
    ...
    def list(self, filter: dict = {}, limit: int = 20, skip: int = 0) -> list:
        ...
        Returns:
            list: The airdrops.
        """
        logging.info("Listing airdrops", extra={"filter": filter})
    ...
    def count(self, filter: dict) -> int:
        ...
        """
        logging.info("Counting airdrops", extra={"filter": filter})
    ...
    def retrieve_by_id(self, airdrop_id: str) -> dict:
        ...
        Returns:
            dict: The airdrop.
        """
        logging.info("Retrieving airdrop", extra={"airdrop_id": airdrop_id})
    ...
    def update(self, airdrop_id: str, body: dict) -> dict:
        ...
        """
        logging.info("Updating airdrop", extra={"airdrop_id": airdrop_id})
    ...
    def archive(self, airdrop_id: str) -> dict:
        ...
        Raises:
            NotFoundError: If the airdrop does not exist.
        """
        logging.info("Archiving airdrop", extra={"id": airdrop_id})
        ...
        airdrop = self.collection.find_one_and_update(
            filter={"_id": airdrop_id},
            update={"$set": {"archived": True, "updated": int(time.time())}},
            upsert=True,
            return_document=ReturnDocument.AFTER,
        )
    ...
    def send(self, airdrop_id: str) -> dict:
        ...
        data = self.collection.find_one_and_update(
            filter={"_id": airdrop_id},
            update={"$set": {"mint_link": mint_link["id"], "status": "complete"}},
            upsert=True,
            return_document=ReturnDocument.AFTER,
        )
    ...
    def get_recipients(self, account_id: str, conditions: list) -> list:
        ...
        """
        logging.info("Getting segment", extra={"conditions": conditions})
        vql = conditions
    ...
    def send_email(self, contact, email_settings, product, mint_link):
        ...
        """
        logging.info("Sending email", extra={"email_settings": email_settings})
        ...
        body = {
            ...
            "message": {
                ...
                "to": [{"email": contact["email"], "type": "to"}],
                "merge_language": "handlebars",
                "global_merge_vars": [
                    {"name": "CLAIM_URL", "content": mint_link["url"]},
                    {"name": "FROM_NAME", "content": from_name},
                    {"name": "FROM_IMAGE", "content": from_image},
                    {"name": "GREETING", "content": greeting},
                    {"name": "MESSAGE", "content": content},
                    {"name": "PREVIEW_TEXT", "content": preview_text},
                    {"name": "PRODUCT_DESCRIPTION", "content": product["description"]},
                    {"name": "PRODUCT_IMAGE", "content": product["image"]},
                    {"name": "PRODUCT_NAME", "content": product["name"]},
                ],
            },
        }
        logging.info(body)
    ...
    def delete(self, airdrop_id: str) -> bool:
        ...
        deleted = self.collection.find_one_and_delete({"_id": airdrop_id})
        if not deleted:
            raise NotFoundError
    ...
    def search(self, account_id, query):
        ...
        Returns:
            list: The airdrops.
        """
        logging.info("Searching airdrops", extra={"query": query})
