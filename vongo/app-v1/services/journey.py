...
from ..utils.sfn import create_state_machine, delete_state_machine, update_state_machine
...
class JourneyService(ExpandableResource):
    ...
    def journey_to_definition(self, journey: dict) -> dict:
        ...
        definition = {
            ...
            "StartAt": "Create journey_run",
            "States": {},
        }

        # Insert create journey_run state
        definition["States"]["Create journey_run"] = {
            "Type": "Task",
            "Resource": Config.STEP_FUNCTION_LAMBDA_ARN,
            "InputPath": "$",
            "Parameters": {
                "state_name.$": "$$.State.Name",
                "task_type": "create_journey_run",
                "journey_id": journey_id,
                "trigger_event.$": "$",
            },
            "OutputPath": "$",
            "Next": "Start journey_run",
        }

        # Insert start state
        definition["States"]["Start journey_run"] = {
            "Type": "Wait",
            "Seconds": 3,
            "Next": start,
        }
        ...
        for name, versify_definition in states.items():
            ...
            if versify_definition.get("end", False):
                next_state = "Save success journey_run"
            else:
            ...
            if action_type == "create_note":
                definition["States"][name] = {
                    ...
                    "OutputPath": "$",
                    "Catch": [
                        {"ErrorEquals": ["States.ALL"], "Next": "Save fail journey_run"},
                    ],
                    "Next": next_state,
                }
                failure_prone = True
                ...
                definition["States"][choice_state_name] = {
                    ...
                    "Default": "Save success journey_run",
                }
        ...
        definition["States"]["Save success journey_run"] = {
            "Type": "Task",
            "Resource": Config.STEP_FUNCTION_LAMBDA_ARN,
            "InputPath": "$",
            "Parameters": {
                "state_name.$": "$$.State.Name",
                "task_type": "update_journey_run",
                "journey_id": journey_id,
                "journey_run_id.$": "$.journey_run_id",
                "status": "completed",
            },
        ...
        definition["States"]["Success"] = {"Type": "Succeed"}

        # Insert failure states
        if failure_prone:
            definition["States"]["Save fail journey_run"] = {
                "Type": "Task",
                "Resource": Config.STEP_FUNCTION_LAMBDA_ARN,
                "InputPath": "$",
                "Parameters": {
            ...
            definition["States"]["Fail"] = {"Type": "Fail"}
