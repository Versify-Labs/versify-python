...
if os.environ.get("ENVIRONMENT", "dev") == "prod":
    ...
    BILLING_URL = (
        "https://dashboard-dev.versifylabs.com/admin/settings/business/billing"
    )
...
class AccountService(ExpandableResource):
    ...
    def create(self, body: dict) -> dict:
        ...
        body["billing"] = {"stripe_customer_id": stripe_customer["id"]}
        body["branding"] = {
        ...
        body["team"] = [{"email": body["email"], "role": "admin"}]
    ...
    def create_member(self, account_id, body: dict) -> dict:
        ...
        new_member = {"email": email, "role": role}
    ...
    def create_billing_session(self, account_id: str) -> dict:
        ...
        session = stripe.billing_portal.Session.create(
            customer=customer_id, return_url=BILLING_URL
        )
        return session
    ...
    def refresh_billing_settings(self, account_id: str) -> dict:
        ...
        for subscription in subscriptions:
            if subscription.get("default_payment_method"):
                billing["stripe_card_brand"] = subscription["default_payment_method"][
                    "card"
                ]["brand"]
                billing["stripe_card_exp_month"] = subscription[
                    "default_payment_method"
                ]["card"]["exp_month"]
                billing["stripe_card_exp_year"] = subscription[
                    "default_payment_method"
                ]["card"]["exp_year"]
                billing["stripe_card_last4"] = subscription["default_payment_method"][
                    "card"
                ]["last4"]
            if subscription.get("latest_invoice"):
                billing["stripe_last_charge_amount"] = subscription["latest_invoice"][
                    "amount_due"
                ]
                billing["stripe_last_charge_at"] = subscription["latest_invoice"][
                    "created"
                ]
            if subscription.get("plan"):
                ...
                billing["stripe_plan_interval_count"] = subscription["plan"][
                    "interval_count"
                ]
            billing["stripe_subscription_cancel_at"] = subscription["cancel_at"]
            billing["stripe_subscription_cancel_at_period_end"] = subscription[
                "cancel_at_period_end"
            ]
            billing["stripe_subscription_period_end"] = subscription[
                "current_period_end"
            ]
            billing["stripe_subscription_period_start"] = subscription[
